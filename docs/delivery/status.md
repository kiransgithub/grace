...
**Clarified MVP update:** [Latest decisions](../mvp-decisions.md) and [verification](../verification-mvp.md) supersede the earlier defaults. The simulator supports user-selected location policy, best-effort queueing and visible trusted admin policy. All 236 distinct tests passed across CI jobs (223 standard plus 13 PostgreSQL); image build and actual Kind pod queue-promotion smoke passed. Durable/stateless GTM deployment requires the planned migration `003` plus its matching runtime; the existing memory service must stay single replica. The tracker now contains 41 tasks.
